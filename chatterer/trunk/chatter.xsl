...
<xsl:stylesheet
    ...
    <xsl:template match="feature[@id='birthdays']">
        <xsl:apply-templates select="title" />
        <xsl:if test="@month">
            <p>If you have a <xsl:value-of select="@month" /> birthday and are not included in the list below, your employee information in Colleague is marked for privacy. If you would like to make sure that your name is on the birthday listings, please check with the Human Resources Office to change the status on your Colleague information.</p>
        </xsl:if>
        <xsl:apply-templates select="*[not(self::title)]" />
    </xsl:template>
