...
class BaseFormatter(Formatter):
    """BaseFormatter is used to define a post_formatter fuction that can be
    shared between the CreditFormatter and the SessionFormatter."""

    def post_formatter(self, value):
        """Attempts to intelligently do some post-processing on the values
        emitted by the formatters.  If the value is a dict, list, tuple or
        None, it is returned unchanged.

        If the value is a bool, it is converted into either 'true' or None,
        depending on its boolean value.

        Otherwise, the value is converted into a unicode string and
        strip()ped."""
...
class CreditFormatter(BaseFormatter):
    ...
    def format_class_sortkey(self, value):
        """A class will only have a sortkey if it is being regrouped.  If not,
        the empty string is returned."""
        subject = mappings.get(
            FormatUtils.get_class_number(self.input), 'subject')
    ...
    def format_course_sortkey(self, value):
        """Lets courses be sorted by their number.  HIST 1301 is sorted as
        1301, for example."""
    ...
    def format_comments(self, value):
        """Comments are given as 10 separate fields.  The algorithm for
        constructing the final comments value is this:
            1.  Join the fields, inserting a space between each field.
            2.  Replace multiple spaces with one space.
            3.  Collapse hyphenated words, since some of the Colleague
                data-entry folks will hyphenate words to break them across
                comments fields."""
        assert type(value) in (list, tuple)
        # join the separate comments fields into a single string
        comments = ' '.join(value)
        # replace multiple spaces with one space
        comments = re.sub('\s{2,}', ' ', comments)
        # collapse hyphenated words
        comments = re.sub('([a-zA-Z]{1})(- )','\\1', comments)
    ...
    def format_class_sortkey_time(self, value):
        ...
        try:
            hours, minutes = start_time[:-2].split(':')
            ampm = start_time[-2:]
            nhours = int(hours)
    ...
    def format_date(self, value):
        """Returns a string representing the same date with any leading zeros
        and the year removed.
    ...
    def format_division(self, value):
        ...
        then based on the current division code.  This search order allows the
        division to be overridden on a rubrik-by-rubrik basis, which lets us
        put PE and Nutrition courses in their proper division.
    ...
    def format_term_sortkey(self, value):
        ...
        create a sortkey."""
        term = FormatUtils.get_term(
            self.input['term'], self.input['start-date'])
    ...
    def format_cross_listings(self, value):
        ...
        for key, patterns in profile.skip_cross_listings.items():
            assert key in self.input,\
                'Invalid key in profile.skip_cross_listings: %s (%s)' % (
                key, repr(profile.skip_cross_listings))
    ...
    def format_special_cross_listings(self, value):
        ...
        if self.input.get('rubrik') == 'ESOL':
            # Basically, ESOL-0051 goes with ESOL-0061, etc.  This is a list
            # of the pairs of course numbers that are to be grouped together.
            esolnumbers = [
                ('0051','0061'),
                ('0052','0062'),
                ('0053','0063'),
            ...
            for pairs in esolnumbers:
                if number in pairs:
                    ...
                    return 'ESOL-%s-%sESOL-%s-%s' % (
                        pairs[0], section, pairs[1], section)

        # we should not group this course
        return None

    def format_minimester(self, value):
        """Generates a minimester name based on the month of the start-date of
        the given class."""
    ...
    def format_minimester_sortkey(self, value):
        """The minimester sortkey is based on the ordinal value of the start
        date of a course in the minimester.  The start date is normalized by
        replacing its day of the month with a 1 (e.g. 08/03/2006 ->
        08/01/2006).

        The ordinal value of the date makes them easily sortable, and
        automatically takes care of the "December needs to come first" problem
        in the Spring semester, for example.

        Without the normalization step, we end up with tons of minimester
        elements, one for each unique start date, which is bad."""
        if FormatUtils.is_minimester(self.input):
            # normalize the start date
            startdate = utils.parsedate(
                self.input['start-date']).replace(day=1)
            # return its ordinal value
            return str(startdate.toordinal())
        return ''

    def format_core_component(self, value):
        """Checks the rubrik and number of the given course against the Core
        Curriculum courses listed in the mappings."""
    ...
    def format_session(self, value):
        ...
        value = SessionFormatter.handle_new_topic_codes(
            value, self.input.get('topic-code',''))
    ...
    def format_subject_comments(self, value):
        comments = FormatUtils.get_comments_for(
            FormatUtils.get_class_number(self.input), 'subject')
        return FormatUtils.post_process_comments(comments)
    def format_subject_sortkey(self, value):
        return FormatUtils.get_sortkey_for(
            FormatUtils.get_class_number(self.input), 'subject')

    def format_topic_name(self, value):
        return FormatUtils.get_name_for(
            FormatUtils.get_class_number(self.input), 'topic')
    ...
    def format_subtopic_name(self, value):
        return FormatUtils.get_name_for(
            FormatUtils.get_class_number(self.input), 'subtopic')
...
class SessionFormatter(BaseFormatter):
    ...
    sub-schemas.  A lot of the actual formatting work is done by the session
    and extra-session formatters in the CreditFormatter, since extra
    information is needed to format some of the session fields."""
    ...
    def format_faculty_last_name(self, value):
        last_name, initial = self.__get_faculty_names(
            self.input.get('faculty-name',''))
        return last_name
    ...
    def __get_faculty_names(self, name):
        """Utility function to split the given name, in "Last Name, First
        Initial" format, into its constituent parts.  Will always return a
        two-tuple, which will contain the last name and first initial or two
        empty strings."""

        # if we have a faculty name and blank start or end times (not just
        # TBA), we are dealing with an extra session that should not be
        # included
        if name.strip() and self.input.get('start-time','').strip() == ''\
                and self.input.get('end-time','').strip() == '':
            return None, None
    ...
    def format_formatted_times(self, value):
        """Tries to convert the start-time and end-time of the given class
        into a nice AP-style string representation."""
    ...
    def add_defaults(self, session_data):
        """Adds default values to the given session_data dict.  Has
        special-case logic for certain fields that the normal SessionFormatter
        formatting functions cannot implement.  Always returns a dict."""
        ...
        if session_data.get('start-time') in ('TBA',) or \
                session_data.get('end-time') in ('TBA',):
        ...
        # Special case for non-lecture and non-lab courses: The
        # formatted-times and room should be 'NA' rather than 'TBA', which is
        # the default for other course types.
        if session_data.get('method', '') not in ('LEC', 'LAB'):
            ...
            if session_data.get('start-time') in default_times and \
                    session_data.get('end-time') in default_times:
...
class FormatUtils:
    """FormatUtils is a class that acts as a namespace for utility functions
    used by the formatting functions in the Formatter classes above.  Each of
    the methods defined in FormatUtils needs to made into a static classmethod
    ...
    def is_minimester(self, classdata):
        ...
        try:
            ...
            # does this class meet the minimum criteria? XXX: the is_flex_hack
            # used below is (supposed to be) an UGLY, TEMPORARY HACK!
            if classtype in ('FD', 'FN') or \
                    weeks < profile.minimester_threshold or \
                    FormatUtils.is_flex_hack(start_date, end_date, term):
                ...
                for key, patterns in profile.skip_minimesters.items():
                    if key in classdata and utils.any(
                        patterns, lambda p: re.match(p, classdata[key])):
                        return False
                    elif key not in classdata:
                        print 'Inavlid key in skip_minimesters: %r' % key

                # all tests passed, so this class qualifies as a minimester
                # class
                return True
    ...
    def is_flex_hack(self, start_date, end_date, term):
        """UGLY HACK!  A hack to include more classes in the Flex section of
        the Spring schedule.  Here is the crux of the problem:

        We need to include more classes in the special minimester or flex
        section of the schedule, which do not meet the criteria outlined above
        in is_minimester.  To do this, we need to know the exact start date
        and time for the term this class is in, which is complicated in the
        Summer schedule, since Colleague sees the May semester as just a part
        of Summer I, but we treat them separately."""
        ...
        # standard start and end dates for this term, so it belongs in the
        # "flex" section
        return True
    ...
    def get_term(self, term_id, class_start):
        ...
        date.  This method is in FormatUtils because it needs to be used in
        multiple formatters and thus should have its results cached.

        If more than one term is given in the current profile's terms dict, we
        loop through each term to see if one has start and end dates which
    ...
    def get_term_dates(self, term):
        ...
        profile's terms dict.  If the given term is not a key in the terms
        dict, the term name is looked up in the term_names dict first."""
        assert term in profile.terms, (
            "Term %s not found in current profile's term dict.  Valid term "
            "names: %s") % (term, ', '.join(profile.terms.keys()))
    ...
    def get_regroupings_for(self, search_key, regrouping_type):
        """Utility function which searches the mappings/regroupings for a set
        of regroupings that matches the pattern defined by search_key with the
        given regrouping_type, which should be one of 'subject', 'topic' or
        'subtopic'.

        Always returns a three-tuple of (name, comments, sortkey) or ('', '',
        '') if no regrouping is found.

        If profile.regroupings is not True, always returns a three-tuple of
        empty strings: ('', '', '').
        """
    ...
    def get_class_number(self, record):
        """A utility function to generate a 'class number', which is a
        combination of the rubrik, number and section."""
    ...
    def post_process_comments(self, value):
        ...
        # UGLY HACK: A flag to tell whether we have already XML-escaped the
        # value
        already_escaped = False

        # apply each regex
        for pattern, replace in regexes:
            # Check to see if we need to XML-escape the given value.  We only
            # need to escape the value if it matches one of the patterns, if
            # it is not already escape and if it is not already an XML
            # fragment (from a "rich" comment in the mappings)
            if re.search(pattern, value) and \
                    not xmlutils.is_xml_fragment(value) and \
                    not already_escaped:
...
class DataError(Exception):
    ...
    def __cmp__(self, other):
        return cmp((self.description, self.course),
                   (other.description, other.course))

    def __str__(self):
        return '%s    #%s    %s' % (
            self.course, self.regnum, self.description)
