...
<xsl:stylesheet
    ...
    <xsl:template name="make-breadcrumbs">
        ...
        <div id="breadcrumbs">
            ...
            <xsl:choose>
            	...
            	<xsl:when test="contains($real-schedule-title, 'Enrolling Soon')">
            		<xsl:variable name="link" select="if (count($parent-names) != 0 or self::special-section or self::minimester) then '../' else './'" as="xs:string" />
            		<a href="{$link}"><xsl:value-of select="$real-schedule-title" /></a>&#160;&#160;&#187;&#160;
            		<!--<xsl:value-of select="if (count($parent-names) = 2) then './bhahaha' else '../eosfhss'" />-->
            	</xsl:when>
                <xsl:otherwise>
            ...
            <xsl:for-each select="$parent-names">
            	...
            	<xsl:variable name="modified-url" select="replace($current-url, 'enrolling/', 'enrolling/soon/')" />
            	<xsl:variable name="final-url" select="if(@name = 'Flex Term') then '../#minimester' else $modified-url" />
                <a href="{$final-url}"><xsl:value-of select="@name" /></a>&#160;&#160;&#187;&#160;
            </xsl:for-each>
    ...
    <xsl:template name="aspx-meta">
        ...
        <xsl:text disable-output-escaping="yes">
            &lt;bhc:meta title="</xsl:text><xsl:if test="$enrolling-now">Enrolling&#8212;</xsl:if><xsl:value-of select="$page-title" /><xsl:text disable-output-escaping="yes">" runat="server" /&gt;</xsl:text>
    </xsl:template>
