<xsl:stylesheet
	...
	<xsl:template match="/schedule">
		<!-- processing vars -->
		<xsl:variable name="base-sem" select="utils:base-semester(term[1]/@semester)"          as="xs:string" />
		<xsl:variable name="base-dir" select="utils:generate-outdir(term[1]/@year, $base-sem)" as="xs:string" />
		<xsl:variable name="outdir"   select="concat($base-dir, '_', $output-type)"            as="xs:string" />
		<xsl:variable name="title"    select="fn:make-title(term/@semester, term/@year)"       as="xs:string" />
		
		<!-- index: create schedule index -->
		<xsl:result-document href="{$outdir}/index.{$ext}">
			<xsl:call-template name="create-index-schedule">
