<xsl:stylesheet
    ...
    <xsl:template match="/schedule">
    	...
    	<!-- processing vars -->
    	<xsl:variable name="year" select="term[1]/@year" as="xs:string" />
    	<xsl:variable name="semester" select="utils:base-semester(term[1]/@semester)" as="xs:string" />
    	<xsl:variable name="dir">
    		<xsl:choose>
    			<xsl:when test="$enrolling-now = 'true'">
    				<xsl:value-of select="concat(utils:generate-outdir($year, $term), '_enrolling')" />
    			</xsl:when>
    			<xsl:otherwise>
    				<xsl:value-of select="concat(utils:generate-outdir($year, $term), '_web')" />
    			</xsl:otherwise>
    		</xsl:choose>
    	</xsl:variable> 
    	
        <!-- full schedule index -->
        <xsl:result-document href="{$dir}/index.{$ext}">
        ...
        <xsl:apply-templates select="term" mode="init">
        	...
        	<xsl:with-param name="dir" select="$dir"  as="xs:string" />
        </xsl:apply-templates>
    </xsl:template>
	...
	<xsl:template match="term[utils:has-classes(.)]" mode="init">
		...
		<xsl:variable name="subdir" select="concat($dir, '/', utils:make-url(@semester))" as="xs:string" />
		...
		<xsl:apply-templates select="descendant::subject" mode="init">
			...
			<xsl:with-param name="dir" select="$subdir" as="xs:string" />
		</xsl:apply-templates>
	...
	<xsl:template match="subject[utils:has-classes(.)]" mode="init">
		<xsl:param name="dir" as="xs:string" />
			...
			<xsl:result-document href="{$dir}/{utils:make-url(@name)}.{$ext}">
				<xsl:call-template name="page-template">
	...
	<xsl:template name="create-special-section">
		...
		<xsl:variable name="subdir"  select="concat($dir, '/', utils:make-url($title))" as="xs:string" />
