<xsl:stylesheet
	...
	<xsl:template match="term" mode="init">
        ...
        <xsl:result-document href="{$path}">
        	...
        	<!-- preamble -->
        	<xsl:call-template name="InDesign-preamble" />
            
            <!-- if multiple terms, output extra info -->
        	<xsl:if test="count(//term) &gt; 1">
                <xsl:value-of select="fn:pStyle('Term Header')" /><xsl:value-of select="@name" /><xsl:call-template name="br" />
                <xsl:value-of select="fn:pStyle('Term Dates')" /><xsl:value-of select="@dates" /><xsl:call-template name="br" />
            </xsl:if>
	...
	<xsl:template name="init-special-section">
		...
		<xsl:result-document href="{$path}/{utils:make-url($title)}.{$ext}">
			...
			<xsl:value-of select="fn:pStyle('Subject Header')" />
	...
	<xsl:template match="subject">
        ...
        <xsl:if test="@name = 'Senior Adult Education Program'">
            <xsl:value-of select="fn:pStyle('Topic Header')" />
	...
	<xsl:template match="class">
        ...
        <xsl:value-of select="fn:pStyle(concat($style-name, ' Class'))" />
	...
	<xsl:template match="meeting[@method = 'COOP']">
		<xsl:value-of select="fn:pStyle('Extra Class')" />
		<xsl:value-of select="@method" /><xsl:call-template name="sep" />
		<xsl:value-of select="'NA'" /><xsl:call-template name="sep" />
		<xsl:choose>
	...
	<xsl:template match="subject/@name | topic/@name | subtopic/@name">
		...
		<xsl:value-of select="fn:pStyle($style-name)" />
	...
	<xsl:template match="type/@name">
		...
		<xsl:if test="normalize-space(.) != normalize-space(ancestor::special-section[1]/@name)">
			<xsl:value-of select="fn:pStyle('Type Header')" /><xsl:value-of select="." /> Courses<xsl:call-template name="br" />
		</xsl:if>
	</xsl:template>
	...
	<xsl:template name="create-special-section">
		...
		<xsl:for-each-group select="$classes" group-by="ancestor::subject/@name">
			...
			<xsl:for-each-group select="current-group()[not(ancestor::topic)]" group-by="ancestor::type/@name">
				...
				<xsl:value-of select="fn:pStyle('Type Header')" />
			...
			<xsl:for-each-group select="current-group()[ancestor::topic]" group-by="ancestor::topic/@name">
				...
				<xsl:if test="current-grouping-key()">
					<xsl:value-of select="fn:pStyle('Special Topic Header')" />
					<xsl:value-of select="upper-case(current-grouping-key())" />
					<xsl:call-template name="br" />
				</xsl:if>
	...
	<!--=====================================================================
		...
		* h1, p, b, i, table, tr, td
		...
		The following templates handle the proper InDesign Tag
    ...
    <xsl:template match="comments//h1">
        ...
        <xsl:value-of select="fn:cStyle(concat($comments-style, ' Header'), current())" />
        <xsl:call-template name="br" />
    </xsl:template>
    ...
    <xsl:template match="comments//b | comments//i">
        ...
        <xsl:value-of select="fn:cStyle(concat($comments-style, $style-suffix), current())" />
    </xsl:template>
    ...
    <xsl:template match="url | email">
        <xsl:value-of select="fn:cStyle('website', current())" />
    </xsl:template>
	...
	<xsl:template name="make-core-list-entry">
		...
		<xsl:if test="count($core-courses) &gt; 0">
			<xsl:value-of select="fn:pStyle('Core List Header')" />
			...
			<xsl:value-of select="fn:pStyle('Core List')" />
	...
	<!-- InDesign preamble -->
	<xsl:template name="InDesign-preamble">
		<xsl:text>&lt;ASCII-MAC&gt;</xsl:text>
		<xsl:call-template name="br" />
		
		<xsl:text>&lt;Version:3&gt;</xsl:text>
		<xsl:text>&lt;FeatureSet:InDesign-Roman&gt;&lt;ColorTable:=&lt;Black:COLOR:CMYK:Process:0,0,0,1&gt;&gt;</xsl:text>
		<xsl:call-template name="br" />
	</xsl:template>
	...
	<!--=====================================================================
		InDesign Tags Functions
		
		insert InDesign Tags into the output
		======================================================================-->
	
	<!-- paragraph styles (look like '<ParaStyle:[name]>') -->
	<xsl:function name="fn:pStyle" as="xs:string">
		<xsl:param name="style-name" as="xs:string" />
		
		<xsl:value-of select="concat('&lt;ParaStyle:', normalize-space($style-name), '&gt;')" />
	</xsl:function>
	
	<!-- character styles (look like '<CharStyle:[name]>' -->
	<xsl:function name="fn:cStyle" as="xs:string">
		<xsl:param name="style-name"    as="xs:string" />
	...
	<xsl:function name="fn:TableStart">
		<xsl:param name="rows" as="xs:integer" />
		<xsl:param name="cols" as="xs:integer" />
		
		<xsl:value-of select="concat(fn:pStyle(''), '&lt;TableStart:', $rows, ',', $cols, ':0:0&gt;')" />
	</xsl:function>
	
	<xsl:function name="fn:TableEnd">
		<xsl:text>&lt;TableEnd:&gt;</xsl:text>
	</xsl:function>
	
	<xsl:function name="fn:RowStart">
		<xsl:text>&lt;RowStart:&gt;</xsl:text>
	</xsl:function>

	<xsl:function name="fn:RowEnd">
		<xsl:text>&lt;RowEnd:&gt;</xsl:text>
	</xsl:function>
	
	<xsl:function name="fn:CellStart">
		<xsl:text>&lt;CellStart:1,1&gt;</xsl:text>
	</xsl:function>
	
	<xsl:function name="fn:CellEnd">
